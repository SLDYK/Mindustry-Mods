...
凭空产出**物品 + 液体 + 热量**的资源源，三种输出都能单独设置；物品与液体的**类型也能挑**。
...
| 造价 | 铍 40 + 硅 20 |
| 物品 | 类型可选（默认相织物），速率 0 ~ 10000 个/秒（默认 10）|
| 液体 | 类型可选（默认水），速率 0 ~ 10000 单位/秒（默认 100）|
| 热量 | 0 ~ 1000 热量单位（默认 0，不开热）|
| 输出方式 | 物品用传送带接走、液体用管道接走；热量**只能传给紧邻的建筑** |
...
1. 在埃里克尔的**运输**分类里找到「资源源」，旁边接上传送带与管道。
2. **点一下方块**打开设置面板：
   - **物品**：在图标格里点一下就能换产出类型（图标格用的就是原版源方块那套选择器，
     会自动过滤掉本星球用不了的物品）；**再点一次已选中的图标 = 关掉这一路输出**；
   - **液体**：同一套图标格，同理；
   - **速率**：三行分别是物品（`0/1/10/100/1k`）、液体（`0/10/100/1k/10k`）、热量（`0/10/50/100/1000`），
     可以直接填数值后按「应用」，也可以点快捷按钮（点快捷按钮会立刻连同输入框里的其他数值一起生效）。
   - 面板顶部会显示当前选中的物品/液体名，没选就显示「（无）」。
3. **双击**方块 = 三种输出全部恢复默认（相织物 10/秒 + 水 100/秒 + 不开热）。
4. 设置的值像其他可配置方块一样会记住：下一个新放的资源源直接沿用。

物品与液体**只产出、不消耗**：没被接走的部分就堆在自己的仓里（200 个物品 / 200 单位液体），
堆满后自动停产；把速率改成 0 只是不再补货，仓里剩下的照样能送出去。
换产出类型时，仓里上一种攒下的会被清掉（否则它们永远送不出去、还会堵住仓）。

**热量不一样**：它不是电网那样的全局网络，而是靠**邻接**传递 —— 只有紧贴着它、并且需要热的建筑
（电热器、熔炉类工厂、热反应堆……）才能拿到，中间隔着别的建筑就传不过去。
原版耗热建筑的需求在 20 ~ 144 热量之间，所以默认的 0 ~ 1000 范围足够；1000 是原版沙盒热源的量级。
...
| 10 | 一个方块可以有多种配置值 | `TypeIO.writeObject` 只认基本类型 / 内容 / `Point2` / `IntSeq` 等。资源源要一次带上「物品类型 + 液体类型 + 两个速率 + 热量」，所以选 `IntSeq`（5 个整数，`-1` = 无），并存成 `config(IntSeq.class, ...)`。另外还注册了 `config(Item.class/Liquid.class)`（互操作用）与 `config(Point2.class)`（1.1.0 的旧格式，旧存档/蓝图还能用）。同一类型只能有一个处理器，但不同类型可以共存 —— `configured()` 是按值的运行时类型查表的。 |
...
| 14 | **热量是邻接传递，不是网络** | 热量没有电网那样的全局图。建筑只要 `implements HeatBlock`（就两个方法 `heat()` / `heatFrac()`），紧邻的耗热建筑就会在自己的 `calculateHeat()` 里直接读它的 `heat()`，但要求同队、且 `!block.rotate || ...` —— 资源源不旋转，所以天然通过。不需要注册任何东西，但也就意味着隔一个格子就传不过去。 |
| 15 | `addBar` 的 Func 返回 `null` 就是不显示该条 | 热量条靠这个实现「只在开了热输出时才显示」（原版 `Block.setBars` 里给液体条也用了这招）。 |
| 16 | 类型选择用原版的 `ItemSelection.buildTable()` | 它对 `liquid` 同样适用（`Seq<T extends UnlockableContent>`），内部已按 `unlockedNow() && isOnPlanet(state.getPlanet()) && !isHidden()` 过滤。传 `block = null, closeSelect = false` 就能把它嵌在自己的面板里、选完不关面板。它用 `button.update(() -> button.setChecked(holder.get() == item))` 回写选中态，而 `programmaticChangeEvents` 默认 false（`private boolean`），所以回写不会反过来触发 `changed()`、不会自己把选择清掉。 |
| 17 | 换产出类型要清旧存量 | 仓里上一种物品/液体在新类型下永远送不出去，会永久堵住仓（送不走 → 停产）。所以 `updateTile()` 里自愈式清理：发现仓里有非当前类型就 `clear()`。注意**不能**在 `read()` 里清 —— `readAll()` 先 `readBase()`（里面把你存档里的物品读进仓）再调 `read()`，在 `read()` 里清会把刚读进来的存量抹掉。 |
| 18 | 整数除法陷阱 | `heatFrac()` 里 `heatOutput / maxHeat` 两个都是 `int`，会永远得 0（只有等于上限时才得 1）。必须写 `heatOutput / (float)maxHeat`。这个 bug 就是离线校验里那条 `heatFrac is heat/maxHeat` 抓出来的。 |
...
三个校验器共 **197 项**，用的是**真实的无头内容加载流程**（`ContentLoader.createBaseContent()`）
把方块真造出来，不启动游戏：

| 校验器 | 项数 | 管什么 |
| --- | --- | --- |
| `VerifyModJar` | 48 | 模组级：`mod.hjson` 位置与字段、主类在包里、**每个内容**的贴图文件名能拼出正确的内容名、两套语言包都定义了该内容用到的每个键（含 `{0}..` 占位符） |
| `VerifyTunableNode` | 41 | 可调电力节点：方块标志、配置往返（含钳制与 `configClear`）、每 tick 发电换算、存档 IO |
| `VerifyResourceSource` | 108 | 资源源：方块标志、三种配置格式（`IntSeq` / 旧 `Point2` / 单个 `Item`/`Liquid`）、`heat()` / `heatFrac()`、**实际跑生产循环**（固定 `Time.delta` 跑 60 帧验算每秒产出）、换类型清仓、关输出清仓、缓冲上限、存档 IO（新 revision 1 **与旧 revision 0 都能读**） |
...
│   └─ ResourceSource.java            # 资源源（物品/液体选择器 + 三种输出、存档）
└─ assets/
